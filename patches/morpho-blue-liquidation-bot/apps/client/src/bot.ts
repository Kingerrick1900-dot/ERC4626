...
import { Scribe } from "./telemetry/Scribe.js";
...
export class LiquidationBot {
  ...
  constructor(inputs: LiquidationBotInputs) {
    ...
    this.scribe = inputs.scribe ?? new Scribe(this.logTag);
  }
  ...
  private async liquidate(position: AccrualPosition) {
    ...
    const pair = `${marketParams.collateralToken.slice(0, 6)}…/${marketParams.loanToken.slice(0, 6)}…`;
    ...
    const healthFactor = position.healthFactor ? Number(position.healthFactor) / 1e18 : null;
    const debtUsd =
      this.pricers && this.pricers.length > 0
        ? ((await this.price(marketParams.loanToken, position.borrowAssets, this.pricers)) ?? null)
        : null;
    ...
    if (
      ...
      this.scribe.recordSkipped(position.user, marketId, "no liquidity route");
      return;
    }
    ...
    try {
      ...
      if (success) {
        this.scribe.recordSuccess(position.user, marketId);
        console.log(`${this.logTag}Liquidated ${position.user} on ${marketId}`);
      } else {
        this.scribe.recordSkipped(position.user, marketId, "not profitable");
        console.log(
          `${this.logTag}ℹ️ Skipped ${position.user} on ${marketId} (not profitable)`,
        );
      }
    } catch (error) {
      this.scribe.recordFailure(
        position.user,
        marketId,
  ...
  private async handleTx(
    ...
    telemetry: { borrower: Address; marketId: Hex },
  ) {
    const functionData = {
      abi: executorAbi,
    ...
    this.scribe.beginSimulation(telemetry.borrower, telemetry.marketId);
    ...
    if (results[1].status !== "success") {
      const simError = String(results[1].error ?? "simulation reverted");
      console.warn(`${this.logTag}Transaction failed in simulation: ${simError}`);
      this.scribe.recordSimulationFailure(telemetry.borrower, telemetry.marketId, simError);
      return false;
    }

    const profitUsd = await this.computeProfitUsd(
    ...
    if (profitUsd === null || profitUsd <= 0) {
      this.scribe.recordSkipped(telemetry.borrower, telemetry.marketId, "not profitable");
      return false;
    }

    this.scribe.recordSimulationSuccess(telemetry.borrower, telemetry.marketId, { profitUsd });
  ...
  private async computeProfitUsd(
    loanAsset: Address,
    loanAssetBalance: {
    ...
    if (this.alwaysRealizeBadDebt && badDebtPosition) return 0;
    if (this.pricers === undefined || this.pricers.length === 0) return 0;
    ...
    if (loanAssetProfit <= 0n) return null;
    ...
    if (loanAssetProfitUsd === undefined || gasUsedUsd === undefined) return null;

    return loanAssetProfitUsd - gasUsedUsd;
  }
  ...
  private async checkProfit(
    ...
    if (profitUsd === null) return false;

...
| Step | Action | Status |
|------|--------|--------|
| 1 | Finish curator package — formalize PA parameters | **DONE** — oracle locked to `dEaD`; see `CURATOR-DOOR-OPS.md` + forum posts |
| 2 | Activate Public Allocator — set `maxIn` to **$700k** | **DONE on King yRSS**; foreign vaults still maxIn=0 |
| 3 | Pull USDC from another market — **$700k** (not $1) | **BLOCKED** on foreign maxIn=0 — forum paste ready |
| 4 | Borrow against PoD → KingVault | **ARMED** — `FirePositionSeed700k` + `watch_maxin_fire.py` |
| Ops | Non-posted inventory → stables | **DONE** — 6.971 USDC → KingVault `0xc2dd5ab3…4c8e` |
| Flash | Named repay source only | **POLICY** — `FLASH-POLICY.md` |

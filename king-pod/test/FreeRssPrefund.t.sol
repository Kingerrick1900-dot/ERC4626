...
contract FreeRssPrefundTest is Test {
    ...
    function test_free_with_prefund() public {
        deal(USDC, HOT, 500e6); // +$500 gap cover
        vm.startPrank(HOT);
        ...
        // Prefund freer with gap cover (king keeps rest)
        IERC20A(USDC).transfer(address(freer), 500e6);
        freer.freeRss();
        vm.stopPrank();
        ...
        assertGt(IERC20A(RSS).balanceOf(HOT), 18_000_000 ether);
    }
}

...
/// @dev Copy of SelfDel1200 for fork proof (same logic as script/FireSelfDel1200.s.sol).
contract SelfDel1200Fork {
...
contract SelfDel200mFork is Test {
    ...
    function test_SelfDel200m_Fork() public {
        vm.createSelectFork(vm.envString("BASE_RPC_URL"));

        (, uint128 borBefore, uint128 collBefore) = IMorpho(MORPHO).position(MID, HOT);
        require(borBefore > 0 && collBefore > 0, "no live book");

        IMorpho.MarketParams memory mp = IMorpho.MarketParams(USDC, RSS, ORACLE, IRM, LLTV);
        uint256 rssBefore = IERC20(RSS).balanceOf(HOT);

        vm.startPrank(HOT);
        SelfDel1200Fork z = new SelfDel1200Fork(MORPHO, USDC, RSS, HOT, MID, mp);
        IMorpho(MORPHO).setAuthorization(address(z), true);
        IERC20(USDC).approve(address(z), 2_000e6);
        z.run();
        IMorpho(MORPHO).setAuthorization(address(z), false);
        vm.stopPrank();

        (uint256 sup, uint128 bor, uint128 coll) = IMorpho(MORPHO).position(MID, HOT);
        assertEq(sup, 0, "sup");
        assertEq(uint256(bor), 0, "bor");
        assertEq(uint256(coll), 0, "coll");
        assertGe(IERC20(RSS).balanceOf(HOT), rssBefore + uint256(collBefore), "rss back");
        console2.log("SELF_DEL_200M_FORK_OK");
    }
}

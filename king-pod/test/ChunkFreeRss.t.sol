...
/// @dev Fork proof: free ~18.5M RSS to hot with $0 USDC prefund; no re-lock in this path.
contract ChunkFreeRssTest is Test {
    ...
    function test_chunk_free_to_king_no_prefund() public {
        ...
        uint256 rssAfter = IERC20A(RSS).balanceOf(HOT);
        console2.log("bor", uint256(bor));
        console2.log("coll", uint256(coll));
        console2.log("rss freed", (rssAfter - rssBefore) / 1e18);

        assertGt(rssAfter, rssBefore + 18_000_000 ether);
        // Dust only — not a recycled full book
        assertLt(uint256(coll), 1_000 ether);
    }
}

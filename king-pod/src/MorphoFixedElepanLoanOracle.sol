...
/// @notice Morpho IOracle: ElepanToken @ Fixed $1 USD, loan (WETH/cbBTC) via UniV3 loan/USDC TWAP.
/// @dev price = loan-raw per 1 collateral-wei, scaled by 1e36 (Morpho).
///      Supports 8dp Elepan (and any collDecimals). Soft peg $1 until backing feed is wired.
contract MorphoFixedElepanLoanOracle {
    uint256 public constant ELEPAN_USD_6 = 1e6; // $1 in USDC raw
    ...
    constructor(
        ...
        require(collDecimals_ > 0 && collDecimals_ <= 18, "COLL_DEC");
    ...
    function price() external view returns (uint256) {
        ...
        // $1 of Elepan → loan_raw = 1e6 * 10^loanDec / usdcPer1Loan
        // price = loan_raw * 1e36 / 10^collDec
        uint256 scale = 10 ** (36 - uint256(collDecimals));
        return FullMath.mulDiv(ELEPAN_USD_6 * (10 ** uint256(loanDecimals)), scale, usdcPer1Loan);
    }

...
contract CrownLeverageExtractor {
    ...
    function curatorDiskFill(uint256 amount) external {
        ...
        // Drain WETH→0 (1-wei stub rounds to 0 on deep books and bricks realloc).
        allocs[0] = IMetaMorphoX.MarketAllocation({marketParams: wethMp, assets: 0});
    ...
    function _borrowIdle(IMorphoX.MarketParams memory ele, uint256 borrowAmt) internal returns (uint256 borrowed) {
        ...
        if (idle == 0) return 0;
        // Keep $1 buffer only when idle is large enough; dust asks must still clear.
        uint256 buffer = idle > 1e6 + 1 ? 1e6 : 0;
        if (idle <= buffer) return 0;
        uint256 maxDraw = idle - buffer;

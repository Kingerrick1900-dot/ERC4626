...
contract KingMoneyMarket is Ownable, ReentrancyGuard {
    ...
    /// @notice Owner releases LP collateral (e.g. to burn on KingPair for USDC).
    function releaseCollateral(address user, uint256 lpAmount, address to) external onlyOwner {
        require(lpAmount > 0 && to != address(0), "ZERO");
        uint256 coll = collateralLp[user];
        require(coll >= lpAmount, "COLL");
        collateralLp[user] = coll - lpAmount;
        require(pair.transfer(to, lpAmount), "LP");
        emit CollateralPulled(user, lpAmount);
    }
}

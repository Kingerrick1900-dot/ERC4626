...
///      Env: PRIVATE_KEY, ASK_USDC (6dp, default 700_000e6), COLL_RSS (18dp, default 1.2e24),
...
contract FireRssOptionCAtomicSeed is Script {
    ...
    function run() external {
        ...
        require(ask >= 600_000e6 && ask <= 700_000e6, "ASK_RANGE");
        // Default ~1.2M RSS buffer above 700k/0.77 ≈ 909k
        uint256 coll = vm.envOr("COLL_RSS", uint256(1_200_000 ether));
        ...
        // Soft LTV check at $1 oracle: maxBorrow ≈ coll * 0.77 / 1e12 (18dp→6dp)
        uint256 maxBorrow = (coll * 77) / 100 / 1e12;

        bool step2 = rssHot >= coll;
        bool step1 = idle >= ask;
        bool clean = borrowShares == 0;
        bool ltvOk = maxBorrow >= ask;

        console2.log("step1 lender idle", step1 ? 1 : 0);
        console2.log("step2 rss authority", step2 ? 1 : 0);
        console2.log("loan clean", clean ? 1 : 0);
        console2.log("ltv ok", ltvOk ? 1 : 0);
        console2.log("maxBorrow from coll", maxBorrow);

        if (!fire) {
            console2.log(step1 && step2 && clean && ltvOk ? "DRY ARMED" : "DRY BLOCKED - fix failing gates");
            console2.log("DRY RUN - set FIRE=1 only when Step 1 lender idle is live");
            return;
        }

        require(step2, "RSS_SHORT");
        require(step1, "LENDER_IDLE_SHORT"); // Step 1 must be live
        require(clean, "EXISTING_BORROW");

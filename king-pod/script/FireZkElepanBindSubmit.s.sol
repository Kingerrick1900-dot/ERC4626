...
/// @notice Submit Elepan-mapped wallet-bind proof to Elepan gate.
/// @dev KING_GO=1 FIRE_ZK_ELEPAN_PROOF=1 GATE=0x... Proof: zk/proofs/elepan_proof_solidity.json
contract FireZkElepanBindSubmit is Script {
    ...
    function run() external {
        ...
        require(vm.envOr("FIRE_ZK_ELEPAN_PROOF", uint256(0)) == 1, "NEED FIRE");
        uint256 pk = vm.envUint("PRIVATE_KEY");
        require(vm.addr(pk) == HOT, "HOT");

        address gateAddr = vm.envAddress("GATE");
        string memory path = string.concat(vm.projectRoot(), "/zk/proofs/elepan_proof_solidity.json");
        ...
        console2.log("ELEPAN_ZK_BIND_LIVE", uint256(1));
    }
}

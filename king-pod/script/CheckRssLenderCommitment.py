...
Pass if idle USDC >= ask (default $500k). Tenor RFQ path is parallel (see FireTenorRssRfq500k.py).

Usage:
  python3 CheckRssLenderCommitment.py
  python3 CheckRssLenderCommitment.py --ask 500000
"""
...
def main() -> int:
    ...
    ap.add_argument("--ask", type=int, default=500_000, help="USDC ask (human)")
    ap.add_argument(
        "--add-borrow",
        action="store_true",
        help="Allow existing Morpho debt (post more RSS + borrow ask)",
    )
    args = ap.parse_args()
    ask_raw = args.ask * 10**6
    if args.ask < 500_000 or args.ask > 700_000:
        print("WARN: plan band is $500k–$700k", file=sys.stderr)
    ...
    # Free RSS for $500k add-borrow (~800k cushion) or clean path (~15M legacy)
    step2_ok = rss >= (800_000 * 10**18 if args.add_borrow else 15_000_000 * 10**18)
    step1_ok = idle >= ask_raw
    loan_clean = pos[1] == 0
    debt_ok = (pos[1] > 0) if args.add_borrow else loan_clean
    ...
    report = {
        ...
        "add_borrow": args.add_borrow,
        "debt_ok": debt_ok,
        "step3_armed": step1_ok and step2_ok and debt_ok,
        "tenor_rfq": "king-pod/script/FireTenorRssRfq500k.py",
        "desks": ["Armitage/Wintermute", "Wintermute OTC", "DWF"],
        "fire_when_ready": (
            "FIRE=1 ASK_USDC=%d ADD_BORROW=%d COLL_RSS=800000000000000000000000 "
            "forge script king-pod/script/FireRssOptionCAtomicSeed.s.sol:FireRssOptionCAtomicSeed "
            "--rpc-url $BASE_RPC --broadcast --slow"
            % (ask_raw, 1 if args.add_borrow else 0)
        ),
    }

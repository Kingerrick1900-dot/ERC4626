...
contract FireScrollEusdExit is Script {
    ...
    function run() external {
        ...
        uint256 recvBefore = IERC20X(USDC).balanceOf(receiver);

        vm.startBroadcast(pk);
        EusdExitSwapper swapper = new EusdExitSwapper(HOT);
        IERC20X(EUSD).approve(address(swapper), swapEusd);
        swapper.swapEusdToUsdc(swapEusd, receiver);
        vm.stopBroadcast();

        uint256 recvAfter = IERC20X(USDC).balanceOf(receiver);
        ...
        console2.log("recvUsdcAfter", recvAfter);
        console2.log("recvDelta", recvAfter > recvBefore ? recvAfter - recvBefore : 0);
        console2.log("hotEusdAfter", hotEusdAfter);
        console2.log("poolUsdcAfter", poolUsdcAfter);
        require(recvAfter > recvBefore, "NO_USDC_OUT");

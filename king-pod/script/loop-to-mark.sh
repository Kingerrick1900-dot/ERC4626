#!/usr/bin/env bash
# Loop the proven $2 elite-flash pattern until Cake vault hits $700k or rails dry.
# Usage: BASE_RPC=... PRIVATE_KEY=... ./script/loop-to-mark.sh
# Re-run whenever USDC lands on King — each run stacks Cake toward the mark.
set -euo pipefail
cd "$(dirname "$0")/.."
RPC="${BASE_RPC:-https://mainnet.base.org}"
MARK_RAW=700000000000
CAKE=0xA1aFcb46a64C9173519180458C1cF302179c832a
USDC=0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913

vault() { cast call "$USDC" "balanceOf(address)(uint256)" "$CAKE" --rpc-url "$RPC" | awk '{print $1}'; }

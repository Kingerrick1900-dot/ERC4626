...
def offers_for_inquiry(inquiry_id: str) -> dict:
    # Public/auth shapes vary — try offersForInquiry then fall back
    q = """
    query($id: ID!) {
      offersForInquiry(quoteInquiryId: $id) {
        items {
          id
          rate
          assets
    ...
    """
    return gql(q, {"id": inquiry_id}, auth=True)
...
def main() -> int:
    ...
    if args.inquiry_id:
        while True:
            try:
                res = offers_for_inquiry(args.inquiry_id)

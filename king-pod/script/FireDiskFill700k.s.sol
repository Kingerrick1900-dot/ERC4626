...
///      Steps: acceptCap if ready → supplyQueue WETH→ELE → deposit USDC → ELE coll → curatorDiskFill → Landing
contract FireDiskFill700k is Script {
    ...
    function run() external {
        ...
        // ELE @ $1, LLTV 77%: coll_8dp >= ask_6dp * 1e2 / 0.77 ≈ ask * 130. Use 200x + floor 1 ELE.
        uint256 needEle = ask * 200;
        if (needEle < 1e8) needEle = 1e8;
        (, uint128 collBefore,) = IMorphoF(MORPHO).position(ELE_USDC, HOT);
        if (uint256(collBefore) < needEle) {
            IERC20F(ELE).approve(address(x), needEle);
            x.depositCollateral(needEle);
            console2.log("COLLATERAL_ELE", needEle);
        }
